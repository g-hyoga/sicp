...
(define (install-polar-package)
	...
	(define (make-from-real-imag x y)
		(cons (sqrt (+ (square x) (square y)))
					(atan y x)))
	;; interface to the rest of the system
	(define (tag x) (attach-tag 'polar x)) (put 'real-part '(polar) real-part) (put 'imag-part '(polar) imag-part) (put 'magnitude '(polar) magnitude) (put 'angle '(polar) angle)
...
#;(define (magnitude z)
	(cond ((rectangular? z)
				...
				(else (error "Unkown type: MAGNITUDE-PART" z))))
#;(define (angle z)
	(cond ((rectangular? z)
				 (angle-rectangular (contents z)))
				((polar? z)
				 (angle-polar (contents z)))
				(else (error "Unkown type: ANGLE-PART" z))))
